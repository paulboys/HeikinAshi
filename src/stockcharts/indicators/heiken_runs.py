...
def compute_ha_run_stats(ha_df: pd.DataFrame) -> dict[str, float | int | str]:
    ...
    Returns:
        Dictionary with keys:
            - run_length: Number of consecutive same-color candles ending at latest.
            - run_percentile: Percentile rank (0-100) of current run vs historical.
            - run_color: 'green' or 'red' for the current run.
            - total_runs: Total number of runs found in history.
...
def _compute_all_run_lengths(colors: pd.Series) -> list[int]:
    ...
    Args:
        colors: Binary series where 1=green, 0=red.
...
def _compute_percentile(value: int, distribution: list[int]) -> float:
    ...
    Args:
        ...
        distribution: All values in the distribution.
