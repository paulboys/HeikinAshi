...
def test_bullish_two_point_divergence():
    ...
    result = detect_divergence(
        df, price_col="Close", rsi_col="RSI", lookback=30, window=3, min_swing_points=2
    )

    # Should detect bullish divergence
    assert (
        result["bullish"] is True
    ), "Expected bullish divergence with price lower low and RSI higher low"
...
def test_bearish_two_point_divergence():
    ...
    assert (
        result["bearish"] is True
    ), "Expected bearish divergence with price higher high and RSI lower high"
