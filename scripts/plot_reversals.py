...
def plot_heiken_ashi_chart(ha: pd.DataFrame, ticker: str, interval: str):
    ...
    colors = ["green" if ha_close >= ha_open else "red" 
    ...
    for i, (date_num, (idx, row)) in enumerate(zip(dates, ha.iterrows())):
        ...
        ax.plot([date_num, date_num], [row["HA_Low"], row["HA_High"]], 
        ...
        ax.add_patch(plt.Rectangle((date_num - width / 2, lower), width, height, 
...
def generate_chart(ticker: str, interval: str = "1d", output_dir: Path = None, show: bool = False) -> bool:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Input CSV file with ticker column"
    )
    parser.add_argument(
        "--interval",
        type=str,
